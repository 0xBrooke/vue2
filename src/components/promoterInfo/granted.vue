<template>
    <div>
        <div class="status">
            <div class="statusTop">
                <span class="status-title">商品状态</span>
                <span
                    :class="productStatus === '0' ? 'activeBtn' : 'btn'"
                    @click="changeStatus('0')"
                >
                    未退货
                </span>
                <span
                    :class="productStatus === '1' ? 'activeBtn' : 'btn'"
                    @click="changeStatus('1')"
                >
                    已退货
                </span>
                <span
                    :class="productStatus === '2' ? 'activeBtn' : 'btn'"
                    @click="changeStatus('2')"
                >
                    全 部
                </span>
            </div>
            <div class="statusBottom">
                <span class="status-title-num">推广工号</span>
                <input
                    type="text"
                    placeholder="请输入推广员工号"
                    v-model="empNoVal"
                    @input="getempNoVal"
                />
                <div class="checkBtn" @click="search">
                    <span>查询</span>
                </div>
            </div>
        </div>
        <div :style="{ height: distance + 'px' }" class="pull_refresh">
            <p v-if="pullStatus !== 2">
                {{ pullTexts[pullStatus] }}
            </p>
            <p class="pull_refresh" v-else>
                <span>{{ pullTexts[pullStatus] }}</span>
            </p>
        </div>
        <div
            class="granted"
            ref="granted"
            @touchstart="touchStart"
            @touchend="touchEnd"
            @touchmove="touchMove"
        >
            <div
                class="granted-content"
                v-for="(item, index) in rewardList"
                :key="index"
            >
                <div class="title-top">
                    <div class="title-top-name">
                        <span>客户 {{ item.cName }}</span>
                    </div>
                    <span class="tel">
                        {{ item.cPhone }}
                    </span>
                </div>
                <div class="granted-info">
                    <div class="granted-title">
                        <span>{{ item.commodityName }}</span>
                        <span class="data">
                            {{ item.contarctsignTime | getTime }}
                        </span>
                    </div>
                    <p>
                        <span class="granted-info-title">订单号：</span>
                        <span>{{ item.orderId }}</span>
                    </p>
                    <p>
                        <span class="granted-info-title">贷款金额:</span>
                        <span>{{ item.commodityLoanAmt }}元</span>
                    </p>
                    <p>
                        <span class="granted-info-title">商品状态:</span>
                        <span v-if="item.commodityStatus === '0'">未退货</span>
                        <span v-if="item.commodityStatus === '1'">退货</span>
                    </p>
                    <p>
                        <span class="granted-info-title">奖励应发:</span>
                        <span>{{ item.rewardAmount }}元</span>
                        实发:
                        <span>{{ item.realSendAmount }}元</span>
                    </p>
                    <p>
                        <span class="granted-info-title">推广员姓名:</span>
                        <span>{{ item.employeeName }}</span>
                    </p>
                    <p>
                        <span class="granted-info-title">工号:</span>
                        <span>{{ item.employeeId }}</span>
                    </p>
                    <p>
                        <span class="granted-info-title">奖励发放时间:</span>
                        <span>{{ item.sendDate | getTime }}</span>
                    </p>
                </div>
            </div>
            <div class="none" v-if="rewardList.length === 0">
                <span>暂无数据</span>
            </div>
            <div class="load-more" v-if="hasMore">
                <span>加载中…</span>
            </div>
            <div class="load-more" v-if="loaded">
                <span>已经到底了~</span>
            </div>
        </div>
    </div>
</template>

<script>
import debounce from 'lodash/debounce';
import { changeTime } from '@/assets/js/utils.js';
export default {
    props: {
        getStatus: {
            type: Function,
            default: null
        }
    },
    data() {
        return {
            rewardList: [],
            empNoVal: '',
            empNo: '',
            productStatus: '2',
            curPagerNo: 1,
            pageSize: 10,
            loaded: false, // 当前页是否请求完成
            hasMore: false, // 是否还有数据
            startPosition: 0, // 触摸开始位置
            endPosition: 0, // 触摸结束位置
            distance: 0, // 下拉距离
            pullStatus: 0, // 0下拉即可刷新  1松手即可刷新  2加载中 3加载完毕
            pullTexts: ['下拉即可刷新哦~', '松手刷新~', '加载中...', '加载完毕']
        };
    },
    mounted() {
        this.$refs.granted.addEventListener(
            'scroll',
            debounce(this.scrollListener, 500)
        );
    },
    methods: {
        //触底事件
        scrollListener() {
            const list = this.$refs.granted;
            if (
                list.scrollHeight - list.scrollTop - list.offsetHeight < 10 &&
                this.hasMore
            ) {
                this.curPagerNo++;
                this.getRewardList();
            }
        },
        touchStart(e) {
            this.startPosition = e.touches[0].pageY;
        },
        touchMove(e) {
            const scrollTop = this.$refs.granted.scrollTop;
            if (scrollTop < 1) {
                let distance = e.touches[0].pageY - this.startPosition;
                if (distance > 0) {
                    e.preventDefault();
                    if (distance > 70) {
                        if (distance !== 70) {
                            this.pullStatus = 1;
                            this.distance = 70;
                        }
                    } else {
                        this.pullStatus = 0;
                        this.distance = distance;
                    }
                }
            }
        },
        touchEnd() {
            const scrollTop = this.$refs.granted.scrollTop;
            if (scrollTop <= 1 && this.distance >= 70) {
                this.pullStatus = 2;
                setTimeout(() => {
                    this.distance = 0;
                    this.pullStatus = 3;
                    this.rewardList = [];
                    this.curPagerNo = 1;
                    this.getRewardList();
                    if (this.getStatus) {
                        this.getStatus();
                    }
                }, 500);
            }
        },
        //选择商品状态
        changeStatus(productStatusVal) {
            this.productStatus = productStatusVal;
            this.rewardList = [];
            this.curPagerNo = 1;
            this.getRewardList();
        },
        //查询
        search() {
            this.empNo = this.empNoVal;
            this.rewardList = [];
            this.curPagerNo = 1;
            this.getRewardList();
        },
        getempNoVal() {
            this.empNo = this.empNoVal;
            if (this.empNo === '') {
                this.rewardList = [];
                this.getRewardList();
            }
        },
        //获取已发放列表
        getRewardList() {
            this.loaded = false;
            this.hasMore = false;
            const data = {
                saCode: this.$store.state.userInfo.personNo,
                sendStatus: '0', //待发放：3；失败：2；发放中：1；已发放：0；
                empNo: this.empNo,
                productStatus: this.productStatus, //商品状态 2全部 1退货 0未退货
                curPagerNo: this.curPagerNo,
                pageSize: this.pageSize
            };
            this.$api('/sm-api/rewardList/query', data).then(res => {
                if (res.code === 0) {
                    this.rewardList.push(...res.data.list);
                    this.hasMore = res.data.rowsCount > this.curPagerNo;
                    if (res.data.rowsCount === this.curPagerNo) {
                        this.loaded = true;
                        this.hasMore = false;
                    }
                } else {
                    Toast(res.showMsg);
                }
            });
        }
    },
    filters: {
        getTime(time) {
            return changeTime(time);
        }
    }
};
</script>

<style lang="scss" scoped>
.status {
    width: 100%;
    height: 110px;
    background: #fff;
    padding: 10px;
    padding-top: 20px;
    div {
        color: #5a6066;
        font-size: 14px;
        text-align: center;
        .status-title {
            padding-right: 54px;
        }
        .status-title-num {
            padding-right: 32px;
        }
        .btn {
            display: inline-block;
            width: 65px;
            height: 28px;
            line-height: 28px;
            text-align: center;
            margin-left: 10px;
            border: 1px solid #dedede;
            border-radius: 4px;
            font-family: PingFangSC-Regular;
            font-size: 14px;
            color: #5a6066;
        }
        .activeBtn {
            display: inline-block;
            width: 65px;
            height: 28px;
            line-height: 28px;
            text-align: center;
            margin-left: 10px;
            background: rgba(10, 156, 112, 0.05);
            border: 1px solid #0a9c70;
            border-radius: 4px;
            font-family: PingFangSC-Regular;
            font-size: 14px;
            color: #0a9c70;
        }
        input {
            caret-color: #0a9c70;
            border: none;
            width: 184px;
            height: 20px;
            line-height: 15px;
            font-size: 14px;
        }
        input::placeholder {
            font-size: 13px;
        }
        .checkBtn {
            display: inline-block;
            opacity: 0.7;
            width: 59px;
            height: 28px;
            background-image: linear-gradient(90deg, #00b07e 0%, #0a9c70 99%);
            border-radius: 14px;
            text-align: center;
            line-height: 29px;
            span {
                color: #fff;
                font-size: 13px;
            }
        }
    }
    .statusTop {
        border-bottom: solid 1px #ebeced;
        padding-bottom: 11.5px;
    }
    .statusBottom {
        padding-top: 11.5px;
    }
}
.granted {
    padding: 0px 15px;
    padding-bottom: 200px;
    height: 100%;
    width: 100%;
    background: #f4f5f7;
    position: fixed;
    overflow: scroll;
    -webkit-overflow-scrolling: touch;
    &::-webkit-scrollbar {
        display: none;
    }
    .granted-content {
        width: 345px;
        background: #fff;
        border-radius: 5px;
        position: relative;
        .title-top {
            height: 44px;
            margin: 10px 15px 0 15px;
            border-bottom: 1px solid #dedede;
            display: flex;
            justify-content: space-between;
            .title-top-name {
                display: inline-block;
                line-height: 44px;
                span {
                    font-size: 14px;
                    color: #5a6066;
                    padding-right: 5px;
                }
            }
            .tel {
                display: inline-block;
                font-size: 14px;
                color: #5a6066;
                line-height: 44px;
            }
        }
        .granted-info {
            padding: 0px 15px 15px 15px;
            .granted-title {
                display: flex;
                padding: 12px 0 8px 0;
                line-height: 18px;
                width: 100%;
                span {
                    display: inline-block;
                    width: 80%;
                    font-size: 14px;
                    font-weight: 550;
                    color: #262c32;
                }
                .data {
                    width: 27%;
                    display: inline-block;
                    font-size: 14px;
                    color: #5a6066;
                    font-weight: 400;
                    height: 20px;
                    text-align: right;
                }
            }
            p {
                color: #7a7f85;
                font-size: 12px;
                line-height: 22px;
                .granted-info-title {
                    display: inline-block;
                    width: 80px;
                }
                span {
                    color: #7a7f85;
                    font-size: 12px;
                    line-height: 20px;
                }
            }
        }
    }
}
.load-more {
    width: 100%;
    height: 50px;
    background: #f3f5f7;
    color: #999999;
    font-size: 12px;
    text-align: center;
    line-height: 50px;
}
input::placeholder {
    color: #b3b8bd;
    font-size: 14px;
}
.none {
    width: 200px;
    height: 115px;
    background: url('../../assets/img/customer/noneCustomer.png') no-repeat;
    background-size: 200px 115px;
    margin: 122px 88px 25px 78px;
    position: relative;
    span {
        font-size: 16px;
        color: #5a6066;
        font-family: PingFangSC-Regular;
        font-weight: 500;
        position: absolute;
        top: 120px;
        left: 66px;
    }
}
.pull_refresh {
    width: 100%;
    background: #f3f5f7;
    color: #999999;
    font-size: 12px;
    text-align: center;
    line-height: 50px;
}
</style>
